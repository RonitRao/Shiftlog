...
export function ChartContainer({ config, children, className }) {
  return (
    <div className={`w-full h-full ${className}`}>
      {/* Dynamic inline standard variable injectors mapping hex colors natively */}
...
export function ChartTooltipContent({ active, payload, hideLabel, indicator }) {
  if (!active || !payload || !payload.length) return null
  return (
    <div className="bg-zinc-950 border border-zinc-800 p-2.5 rounded-xl text-xs font-mono shadow-2xl space-y-1">
